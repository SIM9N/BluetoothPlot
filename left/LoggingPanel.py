...
class LoggingPanel(LabelFrame):
    ...
    def __init__(self, master, controller):
        ...
        self.noteLabel.place(
            relwidth=0.25, relheight=self.relHeight, x=self.column1_x, y=self.row2_y
        )
        ...
        self.note.place(relwidth=0.65, relheight=self.relHeight, x=82, y=self.row2_y)

        self.openBtn = ttk.Button(
            self, text="open", command=self.openExcelFile, style="my.TButton"
        )
        self.openBtn.place(
            relwidth=0.3, relheight=self.relHeight, x=self.column1_x, y=self.row3_y
        )
    ...
    def export2xlsx(self):
        ...
        for i in range(len(self.controller.data[0]) - 1):
            ws.append(
                self.controller.data[j][i + 1] for j in range(len(self.controller.data))
            )
    ...
    def autoExport(self):
        ...
        ws.append(["note", self.note.get()])
        for i in range(self.exportStartIndex, self.exportEndIndex):
            ws.append(
                self.controller.data[j][i] for j in range(len(self.controller.data))
            )
        ws.append([])
        wb.save(self.filename)
        print("saved " + self.filename)
